...
A partir do valor do p-value que é menor que o risco alfa máximo de 5%, podemos dizer que os vinhos Brancos e Tintos tem valores médios de notas diferentes, já que o p-value < 2.2e-16, sendo a médio dos vinhos brancos maior que os tintos (5,87 e 5,63 respectivamente)
...
Banco de dados criado, com todas as variaveis, inclusive mantivemos a quality, e agora com uma variavel dummy "Bom_ruim", onde os vinhos de quality baixa (3,4 e 5) ficaram com 0, as quality 6 foram tiradas do banco de dados e as quality altas (7,8 e 9) ficaram com 1
...
Estes gráficos entre todas as variaveis independentes e a dummy Bom_ruim deve nos dar uma visão das variaveis independentes que tem maior impacto na dummy.
O gráficos nào foram muito conclusivos pois existem malores de 0 e 1 para Bom_ruim em todo o range de valores em todas as variaveis independentes.


Com isso vamos para a regressão logistica onde podemremos, olhando todas as variaveis simultaneamente (diferente dos gráficos), identificar as variaveis mais significativas atraves dos p-valores 
...
predict0 <- predict(Modelo_RegLog0)

```

Análise:

Temos um modelo de Rgressão Logistica com algumas variaveis com p-valor > 5% (citricacid, chlorides, freesulfurdioxide e totalsulfurdioxide)

Este modelo também apresenta os paramentros que ajudam a avaliar a qualidade do modelo:
Residual deviance: 2116.1 e 
AIC: 2140.1
Que quanto menor melhor a qualidade do modelo

Com base nisso, fazer avaliar o modelo de Regressão Logística sem as 4 variaveis não significativas e verificar qual fica melhor  
...
Temos um modelo de Rgressão Logistica com nenhuma variaveis com p-valor > 5%

Este modelo também apresenta os paramentros que ajudam a avaliar a qualidade do modelo:
Residual deviance: 2128.6 e 
AIC: 2144.6

Como os valores de Residual deviance e AIC são praticamente os mesmo do modelo completo (com todas as variaveis independentes), decidimos ficar com este modelo que tem menos variaveis


Agora vamos analisar a tabela de confundimento, os paramentros de acurácia, sensibilidade e outros.

Vamos também olhar o Gráfico ROC, com a sua area

```{r}
predict1 <- predict(Modelo_RegLog1)


confundimento1 <- table(white2$Bom_ruim, predict1 > 0.5)

print("Tabela de Confundimento")
print(confundimento1)

vp1 <- c(confundimento1 [1,1])
fp1 <- c(confundimento1 [1,2])
fn1 <- c(confundimento1 [2,1])
vn1 <- c(confundimento1 [2,2])

neg1 <- (fn1+vn1)
pos1 <- (vp1+fp1)

acuracia1= (vp1 + vn1) / (pos1 + neg1)
sensibilidade1 = vp1 / (vp1 + fn1)
especificidade1 = vn1 / (vn1 + fp1)
eficiencia1 = (sensibilidade1 + especificidade1) / 2
valor_preditivo_positivo1 = vp1 / (vp1 + fp1)
valor_preditivo_negativo1 = vn1 / (vn1 + fn1)

print ("Acuracia"); acuracia1
print ("Sensibilidade"); sensibilidade1
print ("Especificidade"); especificidade1
print ("Eficiencia"); eficiencia1
print ("Valor Preditivo Positivo"); valor_preditivo_positivo1
print ("Valor Preditivo Negativo"); valor_preditivo_negativo1


library(pROC)

roc1 <- roc (white2$Bom_ruim, predict1, plotit = TRUE, add.roc = FALSE)
plot.roc (white2$Bom_ruim, predict1, plotit = TRUE, add.roc = FALSE)
roc1
ci(roc1)
...
Para o modelo com menos variaveis independentes, calculamos a matriz de confundimento que mostra no nosso modelo de regressão logistica as vezes que ele acerta a classificação dos vinhos (bom = bom e ruim = ruim) e também os erros (falso possitivo e falso negativo)

E os indicadores de Acuracia, Sensibilidade e outros

Fizemos o Gráfico ROC, que apresenta boa curvatura e area abaixo da curva de 0.8963, indicando um bom modelo

Agora vamos fazer a arvore de decisão e com estes parametros que acabamos de calcular, vamos avaliar qual o melhor modelo de classificação para esta base de dados, regressão logistica ou arvore de decisão
...
predict2 <- predict(Arvore_decisao1)


confundimento2 <- table(white2$Bom_ruim, predict2 > 0.5)

print("Tabela de Confundimento")
print(confundimento1)

vp2 <- c(confundimento2 [1,1])
fp2 <- c(confundimento2 [1,2])
fn2 <- c(confundimento2 [2,1])
vn2 <- c(confundimento2 [2,2])

neg2 <- (fn2+vn2)
pos2 <- (vp2+fp2)

acuracia2= (vp2 + vn2) / (pos2 + neg2)
sensibilidade2 = vp2 / (vp2 + fn2)
especificidade2 = vn2 / (vn2 + fp2)
eficiencia2 = (sensibilidade2 + especificidade2) / 2
valor_preditivo_positivo2 = vp2 / (vp2 + fp2)
valor_preditivo_negativo2 = vn2 / (vn2 + fn2)

print ("Acuracia"); acuracia2
print ("Sensibilidade"); sensibilidade2
print ("Especificidade"); especificidade2
print ("Eficiencia"); eficiencia2
print ("Valor Preditivo Positivo"); valor_preditivo_positivo2
print ("Valor Preditivo Negativo"); valor_preditivo_negativo2


library(pROC)

roc2 <- roc (white2$Bom_ruim, predict2, plotit = TRUE, add.roc = FALSE)
plot.roc (white2$Bom_ruim, predict2, plotit = TRUE, add.roc = FALSE)
roc2
ci(roc2)
...
Como na regressão logística, agora para a ametriz de decisão, calculamos a matriz de confundimento que mostra no nosso modelo de regressão logistica as vezes que ele acerta a classificação dos vinhos (bom = bom e ruim = ruim) e também os erros (falso possitivo e falso negativo)

E os indicadores de Acuracia, Sensibilidade e outros


Fizemos o Gráfico ROC, que apresenta boa curvatura e area abaixo da curva de 0.8951, indicando um bom modelo


Vamos comparar os indicadores entre os dos métodos (regressão logistica e Arvore de decisão)

```{r}

print("Delte entre Regressão Logisitca e Arvore de Decisão")

print ("Acuracia"); acuracia1 - acuracia2
print ("Sensibilidade");  sensibilidade1 - sensibilidade2
print ("Especificidade"); especificidade1 - especificidade2
print ("Eficiencia"); eficiencia1 - eficiencia2
print ("Valor Preditivo Positivo"); valor_preditivo_positivo1 - valor_preditivo_positivo2
print ("Valor Preditivo Negativo"); valor_preditivo_negativo1 - valor_preditivo_negativo2
print("Area abaixo do Grafico ROC"); roc1$auc - roc2$auc

```

Análise:

Para estes dados, não existe um método que tenha todos os indicadores melhores na comparação
A regressão logística é melhor no:
- Especificidade
- Valor preditivo positivo
- Area abaixo do Gráfico ROC

Enquanto a arvore de decisão é melhor no:
- Acuracia
- Sensibilidade
- Eficiencia
- Valor Preditivo Negativo

Se escolher como critério a qualidade do modelo para escolher vinhos bons, escolheríamos a Regressão Logisitca pela area abaixo do Gráfico ROC e pelo Valor Preditivo Positivo, apesar da pequena diferença entre os modelos
